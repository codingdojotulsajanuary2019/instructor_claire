...
# For y, how would you change the value 20 to 30?
z[0]['y'] = 30
...
def iterDictionary(student_list):
        for student in student_list:
            for keys, values in student.items():
                print(f"{keys} - {values}, ", end="")
            print("\n")

def iterDictionary2(student_list):
    for student in student_list:
...
def iterDictionary3(spec_key,student_list):
    for student in student_list:
...
def instructors_dojos(dojo_list):
    for item, v in dojo_list.items():
        print(f"{len(dojo[item])} {item.upper()}")
        for i in v:
            print(i)
